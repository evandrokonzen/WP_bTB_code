...
double logPostThetasRhos(arma::vec& thetas,
  ...
  double thetasLogPriorWithJac = 0.0;
  double rhosLogPriorWithJac = 0.0;
  double thetaLogit = 0.0;
  double rhoLogit = 0.0;
  for (int iTest=0; iTest<numTests; iTest++) {
    thetaLogit = logitD(thetas[iTest]);
    rhoLogit = logitD(rhos[iTest]);
    thetasLogPriorWithJac += hp_theta[0] * thetaLogit - 
      (hp_theta[0] + hp_theta[1])*log(1 + exp(thetaLogit));
    rhosLogPriorWithJac += hp_rho[0] * rhoLogit - 
      (hp_rho[0] + hp_rho[1])*log(1 + exp(rhoLogit));
  }
  double logPost = logLik + thetasLogPriorWithJac + rhosLogPriorWithJac;
