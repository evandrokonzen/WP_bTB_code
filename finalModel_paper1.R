...
parNamesPlot[[G+2]] <- bquote(beta)
parNamesPlot[[G+3]] <- "q"
parNamesPlot[[G+4]] <- bquote(tau)
parNamesPlot[[G+5]] <- "a"  # bquote(a[2])
parNamesPlot[[G+6]] <- "b"  # bquote(b[2])
parNamesPlot[[G+7]] <- "c"  # bquote(c[1])
for(j in 1:7){
  hpAll[[j]] <- hp[[j]]
}

for(j in 1:numNuTimes){
  parNamesPlot[[G+7+j]] <- bquote(nu[E*","*.(as.numeric(j-1))])  
  hpAll[[7+j]] <- hp[["hp_nu"]] # hp[["hp_nu"]] is the vector for S,E,I cases
}
for(j in 1:(numNuTimes)){
  parNamesPlot[[G+7+numNuTimes+j]] <- bquote(nu[I*","*.(as.numeric(j-1))])  
  hpAll[[7+numNuTimes+j]] <- hp[["hp_nu"]] # same
}

xiPos <- 7+2*numNuTimes+1   # xi position

parNamesPlot[[G+xiPos]] <- bquote(xi)
hpAll[[xiPos]] <- hp[["hp_xi"]]

for(j in 1:numTests){
  parNamesPlot[[G+xiPos+j]] <- bquote(.(testNames[j])~"sens I")
  hpAll[[xiPos+j]] <- hp[["hp_theta"]]
}
for(j in 1:numTests){
  parNamesPlot[[G+xiPos+numTests+j]] <- bquote(.(testNames[j])~"sens E")
  hpAll[[xiPos+numTests+j]] <- hp[["hp_rho"]]
} 
for(j in 1:numTests){
  parNamesPlot[[G+xiPos+2*numTests+j]] <- bquote(.(testNames[j])~"spec")
  hpAll[[xiPos+2*numTests+j]] <- hp[["hp_phi"]]
} 
for(j in 1:numSeasons){
  parNamesPlot[[G+xiPos+3*numTests+j]] <- bquote(eta[.(j*1.0)])
  hpAll[[xiPos+3*numTests+j]] <- hp[["hp_eta"]]
}


hp_xi_mean <- dfTimes$time[hp[["hp_xi"]][1]]
hp_xi_sd <- hp[["hp_xi"]][2]/4
